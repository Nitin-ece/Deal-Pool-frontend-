...
export const SwarmCursor: React.FC<SwarmCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const getColors = () => {
      ...
      const defaultColor = isDark ? "#ffffff" : "#090d14";
      const defaultAccent = isDark ? "#10b981" : "#059669";
      const c = propsRef.current.color || defaultColor;
      const a = propsRef.current.accentColor || defaultAccent;
      return { col: hexToRgb(c), acc: hexToRgb(a) };
    };
    ...
    const compProgram = new Program(gl, {
      ...
      uniforms: {
        tField: { value: null },
        uColor: { value: initialColors.col },
        uAccent: { value: initialColors.acc },
    ...
    container.addEventListener("pointerdown", onDown as any);
    ...
    const frame = (now: number) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (flowMix > 0.001) {
          ...
          const bt = t * 0.22;
          const coarseX =
            (noise3(perm, bx, by + eps, bt) - noise3(perm, bx, by - eps, bt)) / (2 * eps);
          const coarseY =
            -(noise3(perm, bx + eps, by, bt) - noise3(perm, bx - eps, by, bt)) / (2 * eps);

          const fx = px[i] * fineScale + noiseX[i];
          const fy = py[i] * fineScale + noiseY[i];
          const ft = t * 0.55;
          const fineX =
            (noise3(perm, fx, fy + eps, ft) - noise3(perm, fx, fy - eps, ft)) / (2 * eps);
          const fineY =
            -(noise3(perm, fx + eps, fy, ft) - noise3(perm, fx - eps, fy, ft)) / (2 * eps);
      ...
      compProgram.uniforms.uColor.value = dynamicColors.col;
      compProgram.uniforms.uAccent.value = dynamicColors.acc;
