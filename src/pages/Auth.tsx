...
export function Auth({ initialMode = "login" }: AuthProps) {
  ...
  return (
    <div className="relative min-h-dvh bg-[var(--paper)] text-[var(--ink)] antialiased overflow-hidden flex flex-col justify-center items-center px-4 py-12">
      <div className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center flex flex-col items-center">
          <BrandMark size="md" />
          <h1 className="mt-6 font-display text-2xl font-bold tracking-tight text-[var(--ink)]">
            {mode === "login" ? "Welcome back" : "Create your DealPool account"}
          </h1>
          <p className="mt-2 text-xs text-[var(--muted)] max-w-xs">
            {mode === "login"
              ? "Access your escrow wallet, contracts, and active neighborhood deals."
              : "Includes 1,000 free signup grant coins in your secure escrow wallet."}
          </p>
        </div>
        ...
        <motion.div
          ...
          className="rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-6 sm:p-8 shadow-sm"
        >
          {/* Segmented Mode Switcher */}
          <div className="mb-6 flex rounded-full border border-[var(--line)] bg-[var(--paper)] p-1">
            <button
              type="button"
              onClick={() => {
                setMode("login");
              ...
              className={cn(
                "flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-xs font-bold transition-all cursor-pointer",
                mode === "login"
          ...
          {displayError && (
            <div
              role="alert"
              className="mb-5 flex items-start gap-2.5 rounded-2xl border border-rose-500/30 bg-rose-500/10 px-4 py-3 text-xs leading-relaxed text-rose-500"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border border-[var(--line)] bg-[var(--paper)] px-4 py-2.5 text-sm text-[var(--ink)] outline-none transition focus:border-[var(--ink)] focus:bg-[var(--surface)]"
              />
            </div>
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between gap-2">
                ...
                <span className="shrink-0 text-[10px] font-mono text-[var(--muted)]">Min 6 chars</span>
              </div>
              <input
                id="auth-password"
                type="password"
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-full bg-[var(--ink)] py-3 text-xs font-bold text-[var(--paper)] transition-all hover:opacity-90 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer shadow-2xs"
            >
              {isLoading ? (
                "Connecting…"
              ) : mode === "login" ? (
          ...
          <div className="relative my-6 flex items-center justify-center">
            ...
            <span className="relative bg-[var(--surface)] px-3 text-[10px] font-mono font-bold uppercase tracking-wider text-[var(--muted)]">
              or continue with
            </span>
          </div>
          ...
          <button
            ...
            disabled={isLoading || googleLoading}
            className="flex w-full items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-[var(--surface)] py-2.5 text-xs font-bold text-[var(--ink)] transition hover:bg-[var(--line)]/30 active:scale-95 disabled:opacity-50 cursor-pointer shadow-2xs"
          >
        ...
        <div className="mt-6 text-center">
          <Link
            to="/deals"
            className="inline-flex items-center justify-center gap-1.5 text-xs font-semibold text-[var(--muted)] transition hover:text-[var(--ink)]"
          >
            <Compass className="h-3.5 w-3.5" />
            <span>Continue as guest to live radar</span>
          </Link>
        </div>
      </div>
    </div>
