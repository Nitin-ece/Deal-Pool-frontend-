...
export function Admin() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--muted)] mt-0.5">
            Admin oversight on authentication accounts and profile records.
          </p>
        </div>

        <button
          onClick={fetchUsers}
          className="px-4 py-2 rounded-full bg-[var(--surface)] border border-[var(--line)] hover:bg-[var(--line)]/30 text-xs font-bold text-[var(--ink)] shadow-2xs cursor-pointer transition"
        >
          Refresh Directory
        </button>
      </div>
      ...
      {error && (
        <div className="flex items-center gap-2 text-xs text-rose-500 bg-rose-500/10 p-3.5 rounded-2xl border border-rose-500/30">
