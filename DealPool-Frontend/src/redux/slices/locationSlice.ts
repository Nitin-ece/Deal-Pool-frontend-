...
const savedLat = parseFloat(getCookie("user_lat"));
const savedLng = parseFloat(getCookie("user_lng"));
const savedAddress = getCookie("user_address");
const savedCityName = getCookie("user_cityName");

const initialState: LocationState = {
  lat: savedLat || 28.6304,
  lng: savedLng || 77.2177,
  address: savedAddress || "Connaught Place, New Delhi",
  cityName: savedCityName || "New Delhi",
  permission: savedLat ? "granted" : "prompt",
  fetchedAt: savedLat ? Date.now() : null,
  error: null,
};
...
export const locationSlice = createSlice({
  ...
  reducers: {
    setManualLocation: (
      ...
      setCookie("user_lat", String(action.payload.lat));
      setCookie("user_lng", String(action.payload.lng));
      setCookie("user_address", action.payload.address);
      setCookie("user_cityName", state.cityName);
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(requestUserLocation.fulfilled, (state, action) => {
        ...
        setCookie("user_lng", String(action.payload.lng));
        setCookie("user_address", action.payload.address);
        setCookie("user_cityName", action.payload.cityName);
      })
