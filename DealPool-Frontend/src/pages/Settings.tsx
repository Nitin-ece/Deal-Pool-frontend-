    ...
    <div className="max-w-4xl mx-auto px-4 lg:px-8 py-6 space-y-8">
      ...
      <div className="bg-[var(--surface)] rounded-3xl p-6 sm:p-8 border border-[var(--line)] shadow-2xs space-y-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-extrabold text-[var(--ink)] text-base font-display">Security & Authentication</h3>
        </div>
        ...
        {user.has_password === false || user.auth_provider === "google" ? (
          <div className="flex items-start gap-3.5 bg-[var(--paper)] border border-[var(--line)] rounded-2xl p-4 sm:p-5">
            <div className="w-9 h-9 rounded-xl bg-[var(--surface)] border border-[var(--line)] flex items-center justify-center shrink-0 shadow-2xs">
          ...
          <>
            {pwStatus === "success" && (
              <div className="flex items-center gap-2 text-xs text-emerald-500 bg-emerald-500/10 p-3.5 rounded-2xl border border-emerald-500/30 font-semibold">
            ...
            {pwError && (
              <div className="flex items-center gap-2 text-xs text-rose-500 bg-rose-500/10 p-3.5 rounded-2xl border border-rose-500/20">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-4 max-w-md">
              <div>
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-3.5 py-2.5 bg-[var(--paper)] rounded-xl text-sm text-[var(--ink)] border border-[var(--line)] focus:outline-none focus:ring-2 focus:ring-emerald-500/30 transition-all placeholder:text-[var(--muted)]"
                />
              </div>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-[var(--ink)] hover:opacity-90 text-[var(--paper)] text-xs font-bold rounded-full transition-all cursor-pointer disabled:opacity-50 shadow-2xs active:scale-95"
              >
      ...
      {/* Dev-only: Instant Profile Switcher */}
      {import.meta.env.DEV && (
  ...
  return (
    <div className="bg-[var(--surface)] rounded-3xl p-6 sm:p-8 border border-[var(--line)] shadow-2xs space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-extrabold text-[var(--ink)] text-base font-display">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
        <button
          ...
          className="text-left p-3.5 rounded-2xl bg-[var(--paper)] hover:border-[var(--ink)]/40 border border-[var(--line)] transition-all cursor-pointer"
        >
          <div className="flex items-center gap-1.5 font-bold text-xs text-[var(--ink)]">
            <span>Admin Profile</span>
            <span className="text-[9px] bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-1.5 py-0.2 rounded font-bold">
              Admin
            </span>
          </div>
          <div className="text-[10px] text-[var(--muted)] mt-0.5">admin@dealpool.com</div>
        </button>
        ...
        <button
          ...
          className="text-left p-3.5 rounded-2xl bg-[var(--paper)] hover:border-[var(--ink)]/40 border border-[var(--line)] transition-all cursor-pointer"
        >
          <div className="font-bold text-xs text-[var(--ink)]">Riya (Need Requester)</div>
          <div className="text-[10px] text-[var(--muted)] mt-0.5">riya@community.io</div>
        </button>
        ...
        <button
          ...
          className="text-left p-3.5 rounded-2xl bg-[var(--paper)] hover:border-[var(--ink)]/40 border border-[var(--line)] transition-all cursor-pointer"
        >
          <div className="font-bold text-xs text-[var(--ink)]">Arjun (Equipment & Dev)</div>
          <div className="text-[10px] text-[var(--muted)] mt-0.5">arjun@community.io</div>
        </button>
      </div>
    </div>
  );
