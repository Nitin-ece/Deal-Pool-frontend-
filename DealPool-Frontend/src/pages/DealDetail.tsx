    ...
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleShare}
            className="px-3.5 py-1.5 rounded-full border border-[var(--line)] bg-[var(--surface)] hover:bg-[var(--line)]/30 text-xs font-bold text-[var(--ink)] flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
          >
          ...
          {isOwner && (
            <button
              ...
              className="p-2 rounded-full border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 transition-colors cursor-pointer"
              title="Delete Need"
            >
      ...
      {deleteConfirm && (
        <div className="p-4 bg-rose-500/10 border border-rose-500/30 rounded-2xl flex items-center justify-between gap-4 animate-in fade-in">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="px-3 py-1 bg-[var(--surface)] text-[var(--ink)] rounded-lg text-xs font-bold border border-[var(--line)] cursor-pointer"
            >
              Cancel
            </button>
            <button
        ...
        <div className="relative h-64 sm:h-80 lg:h-96 w-full overflow-hidden bg-[var(--paper)]">
          ...
          <div className="absolute bottom-5 left-5 right-5 text-white z-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="space-y-1.5 max-w-2xl">
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight text-white drop-shadow-md leading-tight font-display">
                {currentDeal.title}
              </h1>
            </div>

            <div className="bg-black/75 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-white/20 shrink-0">
          ...
          {acceptSuccess && (
            <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-500 space-y-2 animate-in fade-in">
              ...
              >
                ...
                <span>Go to My Needs & Proposals &rarr;</span>
              </Link>
            </div>
          )}
      ...
      {currentDeal && selectedOfferForAccept && (
        <AcceptOfferModal
          ...
          deal={currentDeal}
          onAccepted={() => {
            setAcceptSuccess("Offer accepted! Contact exchanged and escrow contract activated.");
            if (id) {
              dispatch(fetchDealById(id));
              dispatch(fetchOffersForDeal(id));
            }
