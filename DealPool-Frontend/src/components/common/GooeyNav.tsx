...
export const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const getIndexFromLocation = () => {
    const exactIdx = items.findIndex((it) => {
      if (it.href === location.pathname) return true;
      if (
        it.href === "/deals" &&
        (location.pathname === "/browse" ||
          (location.pathname.startsWith("/deals/") && location.pathname !== "/deals/new"))
      ) {
        return true;
      }
      if (
        it.href === "/deals/new" &&
        (location.pathname === "/deals/new" || location.pathname === "/create-deal")
      ) {
        return true;
      }
      if (it.href === "/my-deals" && location.pathname.startsWith("/my-deals")) return true;
      if (it.href === "/settings" && location.pathname.startsWith("/settings")) return true;
      if (it.href === "/admin" && location.pathname.startsWith("/admin")) return true;
      return false;
    });
    if (exactIdx >= 0) return exactIdx;
    return initialActiveIndex ?? 0;
  };

  const [activeIndex, setActiveIndex] = useState<number>(getIndexFromLocation());
  ...
  useEffect(() => {
    const idx = getIndexFromLocation();
    setActiveIndex(idx);
      ...
      <nav>
        <ul ref={navRef}>
          {items.map((item, index) => {
            ...
            return (
              <li key={item.href || index} className={isSelected ? "active" : ""}>
                <Link
                  to={item.href}
